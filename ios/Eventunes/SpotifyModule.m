...
RCT_EXPORT_METHOD(authenticate:(RCTResponseSenderBlock)callback)
{
  ...
  [[SPTAuth defaultInstance] setClientID:@"aabf13f9376a4bd98b06999bd8d2a46c"];
  
  // The callback (called Custom URL Scheme in XCode project configuration)
  [[SPTAuth defaultInstance] setRedirectURL:[NSURL URLWithString:@"eventunes-spotify://login"]];

  // The scope request for the token
  [[SPTAuth defaultInstance] setRequestedScopes:@[SPTAuthStreamingScope, SPTAuthPlaylistReadPrivateScope, SPTAuthPlaylistReadCollaborativeScope, SPTAuthPlaylistModifyPublicScope, SPTAuthPlaylistModifyPrivateScope]];

  // OPTIONAL. Allows retrieval of refresheable tokens. If not specified, it uses the 'Implicit Grant' auth workflow
  [[SPTAuth defaultInstance] setTokenSwapURL: [NSURL URLWithString:@"https://blooming-caverns-13083.herokuapp.com/swap"]];
